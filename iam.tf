...
# ========================================
# IRSA Roles for Monitoring Components
# ========================================

# Data source for current AWS account
data "aws_caller_identity" "current" {}

# IRSA Role for Loki (S3 Access)
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:sub" = "system:serviceaccount:${var.monitoring_namespace}:loki"
...
# IRSA Role for Prometheus/Mimir (S3 Access for long-term storage)
resource "aws_iam_role" "prometheus_s3" {
  name = "${var.cluster_name}-prometheus-s3-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:sub" = "system:serviceaccount:${var.monitoring_namespace}:prometheus"
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:aud" = "sts.amazonaws.com"
          }
        }
      }
...
resource "aws_iam_policy" "prometheus_s3" {
  name        = "${var.cluster_name}-prometheus-s3-policy"
  description = "IAM policy for Prometheus/Mimir to access S3 bucket"
...
# IRSA Role for Grafana (optional - for CloudWatch datasource, etc.)
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:sub" = "system:serviceaccount:${var.monitoring_namespace}:grafana"
            "${replace(aws_iam_openid_connect_provider.eks.url, "https://", "")}:aud" = "sts.amazonaws.com"
          }
        }
      }
...
resource "aws_iam_policy" "grafana_cloudwatch" {
  ...
  description = "IAM policy for Grafana to read CloudWatch metrics"
...
# ALB Controller Policy (inline for simplicity - use official policy in production)
